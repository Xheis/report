...
Below are snippets of the an implemented Random Search for an MLP classifier. Of note is the parameters included are listed in the table below:
...
\begin{table}[h]
    ...
    \caption{Random Search Hyperparameters}\label{tab:RandCV_HyperOptions}
        \begin{tabular}{lllllll}
            \hline\hline 
            Hyperparameters        & \multicolumn{6}{c}{Options}                                     \\
            \hline 
            'learning\_rate'       & "constant" & "invscaling" & "adaptive" &        &        &      \\
            'hidden\_layer\_sizes' & (145,)     & (147,)       & (149,)     & (151,) & (153), &      \\
...
\begin{lstlisting}
    ...
    clf = RandomizedSearchCV(estimator=mlpc, scoring='f1',param_distributions=parameters,n_jobs=-1,verbose=1,cv=3);
    clf.fit(X_train,y);
