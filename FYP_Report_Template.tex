...
I did:
\begin{itemize}
    \item I learnt and prototyped two machine learning solutions that each take audio and physical data (GPS, accelerometer, gyroscope). They compare whether the physical supplementary data could improve the classification of an acoustic event, as measured by the F1 score.
...
\vspace{-3mm}
The project investigated whether supplementary data (Accelerometer, Gyroscope, GPS) will improve a classifiers ability to classify an "acoustic event" in an acoustic event classification / detection (AEC/D) problem. The motivation behind this was to improve AEC/D without onerous microphone requirements, to enable more widespread commercial use of AEC/D.
\\
...
\sref{sec:LitReview} reviews current research. Presently the field is expanding with broader advances in the wider machine learning community, however AEC/D remains a non-trivial problem with few commercial products, and no real-world AEC/D solution yet. Comparatively to the wider machine learning community, acoustic event detection is very limited in active researchers. This specific issue hasn't been answered. 
\\
...
\sref{sec:Discussion} evaluates the results and discusses the limitations of the results. It will discuss how multi-sensor AEC/D is effective, and why the result will be valid in other projects. The significance of these results throughout the field.
%but requires more datasets to test smaller, more specific tasks. 

\sref{sec:Conclusion} will present how these results will contribute to the field, and reflect on the project.

\sref{sec:futurework} will list future work including; investigations into new datasets with a greater number of labels, and further detailed analysis into the cause of the result.
...
The machine learning field dates back to the first reproduction of a neuron network \cite{mcculloch1943logical} in an attempt to explain the physical decision making process of the nervous system. There were seminal scientific breakthroughs and papers on machine learning from mathematicians and early computer scientists throughout the mid to late 20th century. The advent of cheaper, and faster processing power has revived the idea of machine learning, and the widespread availability of data has again accelerated this boom. Machine learning encompasses any application of a computer learning to complete a task.

\paragraph{Learning Computers}
Machine learning is based on the concept of having a computer \inlineQuote{learn}. This broad term has been used to described fitting a function to historical data, iterative output optimisation based on previous Hypotheses, and `genetic algorithms' (which are a random search, using previous success as an initial guess for a new random guess). All these strategies have a common theme: machine learning rewards progress and punishes mistakes. Often, but not always, this can be reduced down to a cost/constraint model.
...
This definition of machine learning can be applied to this project; the task T (AEC/D), the E (Audio, IMU, GPS), and P (a traditional F1 score). 

% The computer program described in this project has been produced in Python, using the scikit-learn and TensorFlow libraries.

\subparagraph{Machine Learning from a Mechatronics Perspective}
...
The process of training a model relies on having examples of data, paired with a true classification from that known data. This is what is more broadly known as a supervised learning. A key concept for this project is to be comparing the classifier produced, against the recorded truth of the samples. This allowed the comparison of the classifier against 
...
To do this, the project will analyse the difference between two classifiers. One with supplementary data, and one without. %F1 Scores of a binary MLPClassifier, and a multi-label MLPClassifier, both trained on the ExtraSensory dataset using the method described in \sref{sec:Method}.
...
Debriefing is an education tool used to educate an individual, a group or record a more holistic history of an event, for purposes of performance feedback.
...
Debriefing is a key tool in training after any activity, and has been extended to many industry, most notably in the training of doctors \cite{johnson2011debriefing}.

Traditionally, debriefing is done through a collection of primary sources (where available) and secondary sources.\cite{recentAAR} In the information age, these primary sources have expanded to include large datasets, recordings and other digital forensics. This new and increasing range of primary sources provides the potential to significantly improve the steps of debriefing, and ultimately improve debriefing outcomes, which is a process prone to review \cite{PsychologicalDebriefing2012}.
