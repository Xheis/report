...
\end{itemize}
% for us we will be a stage one choosing a dotted be that which is defined in al preprocessing data stage 4.1 we will pre-process our data as described in full point to preprocessing data we will choose a model which will be an MLP classifier which has been selected due to it's verbose robust fitness to potentially why range wide ranges of data and due to our use of tabular data in stage 5 we will validate our data by doing a k fold to pick hyperparameters for the mlpclassifier this is the process of refining what are chosen models parameters are and finally will use the F1 score as a primary metric which will help determine what what our classifier is finally we will then report on these results as well as run through some through user 59 user 60 which has been dedicated to be out final evaluation example
...
The model required for this project 
